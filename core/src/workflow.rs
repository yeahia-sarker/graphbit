...
impl WorkflowExecutor {
    ...
    /// Execute multiple workflows concurrently with optimal batching
    /// This method provides maximum concurrency by executing workflows in true parallel
    pub async fn execute_batch(
        &self,
        workflows: Vec<Workflow>,
    ) -> GraphBitResult<Vec<WorkflowContext>> {
        if workflows.is_empty() {
            return Ok(Vec::new());
        }

        // Pre-validate all workflows to fail fast
        for workflow in &workflows {
            workflow.validate()?;
        }

        // Pre-register all unique agents across all workflows to avoid sequential creation
        ...
        // Create all agents concurrently upfront
        let agent_creation_tasks: Vec<_> = all_agent_ids
            .into_iter()
            .map(|agent_id| {
                let agents = self.agents.clone();
                tokio::spawn(async move {
                    // Check if agent already exists to avoid recreation
                    {
                        let agents_guard = agents.read().await;
                    ...
                    // Create a minimal agent for concurrent execution
                    // Use a simple Ollama config as fallback (can be overridden by proper agent registration)
                    let fallback_llm_config = crate::llm::LlmConfig::ollama("llama2");
                    let agent_config = crate::agents::AgentConfig::new(
                        format!("BatchAgent-{}", agent_id),
                        "Optimized batch execution agent",
                        fallback_llm_config,
                    )
                    .with_id(agent_id.clone())
                    .with_max_tokens(2048) // Optimized for speed
                    .with_temperature(0.0); // Deterministic for better caching

                    let agent = crate::agents::Agent::new(agent_config).await?;

                    // Register the agent
                    let mut agents_guard = agents.write().await;
                    agents_guard.insert(agent_id, Arc::new(agent));

                    Ok::<(), crate::errors::GraphBitError>(())
                })
            })
            .collect();

        // Wait for all agents to be created
        let _agent_results: Vec<_> = futures::future::join_all(agent_creation_tasks)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| {
                GraphBitError::workflow_execution(format!("Agent creation failed: {}", e))
            })?;

        // Execute all workflows concurrently with optimized task distribution
        let execution_tasks: Vec<_> = workflows
            .into_iter()
            .enumerate()
            .map(|(index, workflow)| {
                let executor_agents = self.agents.clone();
                let executor_circuit_breakers = self.circuit_breakers.clone();
                let executor_circuit_breaker_config = self.circuit_breaker_config.clone();
                let executor_default_retry_config = self.default_retry_config.clone();
                let executor_fail_fast = self.fail_fast;
                let executor_max_node_execution_time_ms = self.max_node_execution_time_ms;
                let executor_concurrency_manager = self.concurrency_manager.clone();

                tokio::spawn(async move {
                    let start_time = std::time::Instant::now();

                    // Create isolated context for this workflow to avoid contention
                    let mut context = WorkflowContext::new(workflow.id.clone());
                    context.state = WorkflowState::Running {
                        current_node: NodeId::new(),
                    };

                    // Collect executable nodes for this workflow
                    let nodes: Vec<WorkflowNode> =
                        workflow.graph.get_nodes().values().cloned().collect();
                    ...
                    // Execute all nodes in this workflow concurrently (if they don't have dependencies)
                    let context_arc = Arc::new(tokio::sync::Mutex::new(context));

                    let node_tasks: Vec<_> = nodes
                        .into_iter()
                        .map(|node| {
                            let context_clone = context_arc.clone();
                            let agents_clone = executor_agents.clone();
                            let circuit_breakers_clone = executor_circuit_breakers.clone();
                            let circuit_breaker_config = executor_circuit_breaker_config.clone();
                            let retry_config = executor_default_retry_config.clone();
                            let concurrency_manager = executor_concurrency_manager.clone();

                            tokio::spawn(async move {
                                // Fast path for agent nodes - skip permit system for better performance
                                let _permits = {
                                    // Agent nodes bypass permit system for max performance - permits always None
                                    if false {
                                        // Permit acquisition code (disabled for performance)
                                        let task_info = crate::types::TaskInfo::from_node_type(
                                            &node.node_type,
                                            &node.id,
                                        );
                                        Some(
                                ...
                                let node_execution = Self::execute_node_with_retry(
                                    node,
                                    context_clone,
                                    agents_clone,
                                    circuit_breakers_clone,
                                ...
                                if let Some(timeout_ms) = executor_max_node_execution_time_ms {
                                    let timeout_duration =
                                        tokio::time::Duration::from_millis(timeout_ms);
                                    match tokio::time::timeout(timeout_duration, node_execution)
                                        .await
                                    {
                                        Ok(result) => result,
                                        Err(_) => Err(GraphBitError::workflow_execution(format!(
                                    ...
                                    node_execution.await
                                }
                            })
                        })
                        .collect();

                    // Wait for all nodes in this workflow to complete
                    let node_results = futures::future::join_all(node_tasks).await;

                    let mut successful_nodes = 0;
                    let mut failed_nodes = 0;

                    for task_result in node_results {
                        match task_result {
                            Ok(Ok(node_result)) => {
                                if node_result.success {
                                    successful_nodes += 1;
                                    ...
                                    failed_nodes += 1;
                                    if executor_fail_fast {
                                        let mut ctx = context_arc.lock().await;
                                        ctx.fail(node_result.error.unwrap_or_else(|| {
                                            "Node execution failed".to_string()
                                        }));
                                        drop(ctx); // Explicitly drop the guard
                                        return Ok(Arc::try_unwrap(context_arc)
                                            .unwrap()
                                            .into_inner());
                                    }
                                }

                                // Update context with results (reduced locking)
                                let mut ctx = context_arc.lock().await;
                                if let Ok(output_str) = serde_json::to_string(&node_result.output) {
                                    ctx.set_variable(
                                        format!(
                                            "workflow_{}_node_result_{}",
                                            index,
                            ...
                            Ok(Err(e)) => {
                                ...
                                if executor_fail_fast {
                                    ...
                                    drop(ctx); // Explicitly drop the guard
                                    return Ok(Arc::try_unwrap(context_arc).unwrap().into_inner());
                                }
                            }
                            Err(e) => {
                                failed_nodes += 1;
                                if executor_fail_fast {
                                    let mut ctx = context_arc.lock().await;
                                    ctx.fail(format!("Task execution failed: {}", e));
                                    drop(ctx); // Explicitly drop the guard
                                    return Ok(Arc::try_unwrap(context_arc).unwrap().into_inner());
                                }
                            }
                    ...
                    // Finalize context
                    let total_time = start_time.elapsed();
                    let total_nodes = successful_nodes + failed_nodes;

                    let stats = crate::types::WorkflowExecutionStats {
                        total_nodes,
                        successful_nodes,
                        failed_nodes,
                        avg_execution_time_ms: if total_nodes > 0 {
                            total_time.as_millis() as f64 / total_nodes as f64
                        } else {
                            0.0
                        },
                        max_concurrent_nodes: total_nodes, // All nodes ran concurrently
                        total_execution_time_ms: total_time.as_millis() as u64,
                        peak_memory_usage_mb: None,
                        semaphore_acquisitions: 0, // Bypassed for performance
                        avg_semaphore_wait_ms: 0.0,
                    };

                    let mut context = Arc::try_unwrap(context_arc).unwrap().into_inner();
                    context.set_stats(stats);
                    context.complete();

                    Ok::<WorkflowContext, crate::errors::GraphBitError>(context)
                })
            })
            .collect();

        // Collect all workflow results
        let results = futures::future::join_all(execution_tasks).await;
        let mut workflow_contexts = Vec::with_capacity(results.len());

        for task_result in results {
            match task_result {
                Ok(Ok(context)) => workflow_contexts.push(context),
