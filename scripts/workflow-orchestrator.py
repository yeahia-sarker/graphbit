...
class WorkflowOrchestrator:
    ...
    def _save_status(self):
        ...
        data = {"last_updated": datetime.now().isoformat(), "results": {}}
    ...
    def start_phase(self, phase: WorkflowPhase) -> bool:
        ...
        # Initialize result
        result = WorkflowResult(phase=phase, status=WorkflowStatus.RUNNING, start_time=datetime.now())
    ...
    def get_pipeline_status(self) -> Dict[str, Any]:
        ...
        for phase in WorkflowPhase:
            if phase in self.results:
                ...
                status["phases"][phase.value] = {"status": "not_started", "duration": None, "artifacts_count": 0}
    ...
    def print_status(self):
        ...
        for phase_name, phase_info in status["phases"].items():
            status_icon = {"success": "✅", "failure": "❌", "running": "🔄", "pending": "⏳", "not_started": "⚪"}.get(phase_info["status"], "❓")
...
def main():
    ...
    parser.add_argument("--phase", choices=[p.value for p in WorkflowPhase], help="Workflow phase to operate on")

    parser.add_argument("--success", action="store_true", help="Mark phase as successful (for complete command)")

    parser.add_argument("--artifacts", nargs="*", help="List of artifacts produced by the phase")

    parser.add_argument("--error", help="Error message (for failed completion)")
    ...
    if args.command == "start":
        ...
        phase = WorkflowPhase(args.phase)
        success = orchestrator.complete_phase(phase, args.success, args.artifacts or [], args.error)
        sys.exit(0 if success else 1)
