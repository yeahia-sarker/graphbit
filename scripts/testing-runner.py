...
class TestingRunner:
    ...
    def _run_python_tests(self) -> Tuple[bool, str, Optional[str], Optional[float]]:
        ...
        try:
            ...
            result = subprocess.run(
                ['python', '-m', 'pytest'] + test_paths + ['-v', '--tb=short'],
                capture_output=True,
                text=True,
                cwd=self.root_path,
                timeout=600
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GraphBit Testing Runner",
    ...
    parser.add_argument(
        ...
        help='Platform identifier for testing'
    )
    
    parser.add_argument(
        '--suite',
        choices=['rust-unit', 'rust-integration', 'python-unit', 'python-integration', 'coverage', 'performance', 'all'],
        default='all',
