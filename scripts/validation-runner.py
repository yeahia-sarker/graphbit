...
class ValidationRunner:
    ...
    def _check_python_formatting(self) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            ...
            result = subprocess.run(
                ['python', '-m', 'black', '--check', '--diff', 'python/', 'scripts/'],
                capture_output=True,
                text=True,
                cwd=self.root_path,
                timeout=120
    ...
    def _check_python_linting(self) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            # Try flake8 first
            result = subprocess.run(
                ['python', '-m', 'flake8', 'python/', 'scripts/'],
                capture_output=True,
                text=True,
                cwd=self.root_path,
                timeout=120
            ...
            try:
                result = subprocess.run(
                    ['python', '-m', 'pylint', 'python/', 'scripts/'],
                    capture_output=True,
                    text=True,
                    cwd=self.root_path,
                    timeout=180
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GraphBit Validation Runner",
    ...
    parser.add_argument(
        '--check',
        choices=['formatting', 'linting', 'security', 'dependencies', 'versions', 'docs', 'all'],
        default='all',
