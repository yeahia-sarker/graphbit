...
class WorkflowValidator:
    ...
    def __init__(self, root_path: Path):
        ...
        self.expected_workflows = {
            "01-validation.yml": {
                "name": "01 - Validation",
                "required_jobs": ["validation"],
                "required_triggers": ["push", "pull_request", "workflow_dispatch"]
            },
            "02-testing.yml": {
                "name": "02 - Testing", 
                "required_jobs": ["prerequisites-check", "test-matrix"],
                "required_triggers": ["workflow_dispatch", "workflow_call"]
            },
            "03-build.yml": {
                "name": "03 - Build",
                "required_jobs": ["prerequisites-check", "build-matrix"],
                "required_triggers": ["workflow_dispatch", "workflow_call"]
            },
            "04-release.yml": {
                "name": "04 - Release",
                "required_jobs": ["prerequisites-check", "release-process"],
                "required_triggers": ["workflow_dispatch", "workflow_call"]
            },
            "05-deployment.yml": {
                "name": "05 - Deployment",
                "required_jobs": ["prerequisites-check", "deployment-verification"],
                "required_triggers": ["workflow_dispatch", "workflow_call"]
            },
            "pipeline-orchestrator.yml": {
                "name": "Pipeline Orchestrator",
                "required_jobs": ["initialize-pipeline", "validation-phase"],
        ...
        self.expected_scripts = {
            "workflow-orchestrator.py": {
                "description": "Phase coordination script",
                "required_functions": ["start", "complete", "status", "reset", "check"]
            },
            "validation-runner.py": {
                "description": "Validation execution script",
                "required_functions": ["run_all_validations"]
            },
            "testing-runner.py": {
                "description": "Testing execution script", 
                "required_functions": ["run_all_tests"]
            },
            "build-runner.py": {
                "description": "Build execution script",
                "required_functions": ["run_all_builds"]
            },
            "verify-version-sync.py": {
                "description": "Version management script (legacy)",
                "required_functions": []  # Legacy script, minimal validation
            }
        }
    ...
    def _validate_file_structure(self):
        ...
        required_dirs = [
            self.workflows_dir,
            self.scripts_dir,
            self.root_path / ".github" / "artifacts"
        ]
        ...
        for workflow_file in self.expected_workflows.keys():
            ...
            try:
                ...
                self.issues.append(ValidationIssue(
                    ...
                    category="Workflow Syntax",
                    message=f"Could not read {workflow_file}: {str(e)}",
                    file_path=str(workflow_path)
                ))
        ...
        for workflow_file, expected_config in self.expected_workflows.items():
            ...
            try:
                ...
                self.issues.append(ValidationIssue(
                    level=ValidationLevel.ERROR,
                    category="Workflow Configuration",
                    message=f"Could not validate configuration for {workflow_file}: {str(e)}",
                    file_path=str(workflow_path)
                ))
        ...
        for script_file, expected_config in self.expected_scripts.items():
            ...
            try:
                ...
                self.issues.append(ValidationIssue(
                    level=ValidationLevel.ERROR,
                    category="Script Functionality",
                    message=f"Could not test script {script_file}: {str(e)}",
                    file_path=str(script_path)
                ))
    ...
    def _validate_dependencies(self):
        ...
        try:
            ...
            self.issues.append(ValidationIssue(
                ...
                suggestion="Install required Python packages: pip install pyyaml requests"
            ))
        ...
        try:
            ...
            if result.returncode != 0:
                self.issues.append(ValidationIssue(
                    level=ValidationLevel.ERROR,
                    category="Dependencies",
                    message="Git is not available or not functional",
                    suggestion="Install Git and ensure it's in PATH"
                ))
    ...
    def _validate_permissions(self):
        ...
        test_dirs = [
            self.root_path / ".github" / "artifacts",
            self.root_path / ".github" / "test-artifacts"
        ]
        ...
        for test_dir in test_dirs:
            if test_dir.exists() and not os.access(test_dir, os.W_OK):
                self.issues.append(ValidationIssue(
                    level=ValidationLevel.ERROR,
                    category="Permissions",
                    message=f"Directory {test_dir.name} is not writable",
        ...
        if orchestrator_path.exists():
            try:
                ...
                self.issues.append(ValidationIssue(
                    level=ValidationLevel.ERROR,
                    category="Integration Points",
                    message=f"Could not validate orchestrator integration: {str(e)}",
                    file_path=str(orchestrator_path)
                ))
            ...
            try:
                ...
                for pattern in sensitive_patterns:
                    ...
                    if (f"{pattern}:" in content and
                        "secrets." not in content and
                        "cache" not in content and
                        "github_token" not in content and
                        "${{" not in content):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GraphBit Workflow System Validator",
