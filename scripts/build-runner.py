...
class BuildRunner:
    ...
    def run_all_builds(self) -> bool:
        ...
        for name, build_func in builds:
            ...
            try:
                ...
                result = BuildResult(name=name, passed=passed, duration=duration, artifacts=artifacts, output=output, error=error)
                ...
                duration = time.time() - start_time
                result = BuildResult(name=name, passed=False, duration=duration, artifacts=[], output="", error=str(e))
    ...
    def _save_detailed_report(self):
        ...
        report_data = {
            ...
            "results": [{"name": r.name, "passed": r.passed, "duration": r.duration, "artifacts": r.artifacts, "output": r.output, "error": r.error} for r in self.results],
        }
...
def main():
    ...
    parser.add_argument("--platform", default="ubuntu-latest", help="Platform identifier for building")

    parser.add_argument("--target", default="x86_64", help="Target architecture for building")

    parser.add_argument("--step", choices=["rust-lib", "python-ext", "wheels", "sdist", "docs", "verify", "all"], default="all", help="Specific build step to run")
