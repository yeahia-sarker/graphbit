...
class AdvancedChangelogGenerator:
    ...
    def __init__(self, root_path: Path):
        ...
        self.commit_patterns = {
            ...
            ChangeType.BREAKING: [r"BREAKING CHANGE", r"breaking:", r"major:", r"!:"],
            ChangeType.FEATURE: [r"^feat(\([^)]*\))?:", r"^feature(\([^)]*\))?:", r"^\[Feature\]", r"minor:"],
            ChangeType.FIX: [r"^fix(\([^)]*\))?:", r"^patch:", r"^\[Fix\]", r"^\[Hotfix\]"],
            ChangeType.SECURITY: [r"^security(\([^)]*\))?:", r"^\[Security\]", r"security fix", r"vulnerability"],
            ChangeType.PERFORMANCE: [r"^perf(\([^)]*\))?:", r"^\[Performance\]", r"performance", r"optimization"],
            ChangeType.DOCUMENTATION: [r"^docs(\([^)]*\))?:", r"^\[Documentation\]", r"^\[Doc\]", r"documentation"],
            ChangeType.TESTING: [r"^test(\([^)]*\))?:", r"^\[Test\]", r"^\[Tests\]", r"unit test", r"integration test"],
            ChangeType.CHORE: [r"^chore(\([^)]*\))?:", r"^style(\([^)]*\))?:", r"^refactor(\([^)]*\))?:", r"^build(\([^)]*\))?:", r"^ci(\([^)]*\))?:"],
        }
    ...
    def categorize_commit(self, commit_message: str) -> Tuple[ChangeType, bool, Optional[str]]:
        ...
        breaking_change = any(re.search(pattern, commit_message, re.IGNORECASE) for pattern in self.commit_patterns[ChangeType.BREAKING])
    ...
    def generate_changelog_entries(self, commits: List[Dict]) -> Dict[ChangeType, List[ChangelogEntry]]:
        ...
        entries_by_type: Dict[ChangeType, List[ChangelogEntry]] = {change_type: [] for change_type in ChangeType}

        for commit in commits:
            message = commit["message"]

            # Skip merge commits and version bump commits
            if message.startswith("Merge ") or "bump version" in message.lower() or "chore: bump version" in message.lower():
                continue
    ...
    def generate_full_changelog_entry(self, version: str, entries_by_type: Dict[ChangeType, List[ChangelogEntry]]) -> str:
        ...
        breaking_changes = sum(len([e for e in entries if e.breaking_change]) for entries in entries_by_type.values())
        ...
        if total_changes > 0:
            ...
            for section in sorted_sections:
                if section.entries:
                    ...
                    category_name = section.title.split(" ", 1)[1]
