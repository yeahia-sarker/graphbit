"""Run high-performance concurrent tasks using Mistral with Ollama."""
...
def run_high_performance_concurrent_tasks_mistral():
    """Run high-performance concurrent tasks using Mistral with Ollama."""
    print("[LOG]  Running High-Performance Concurrent Tasks with Mistral...")
    ...
    print("\n[LOG]  Attempting native concurrent execution...")
    try:
        results = executor.execute_concurrent_agent_tasks(CONCURRENT_TASK_PROMPTS, agent_id)
        for i, result in enumerate(results):
            task = CONCURRENT_TASK_PROMPTS[i]
            task_name = f"Native Task {i+1}"

            if not result.startswith("ERROR:"):
                output = result
                tokens = count_tokens_estimate(task + output)
                total_tokens += tokens
                successful += 1
                preview = output[:200].replace("\n", " ")
                print(f"{task_name} → {preview}... [Tokens: {tokens}]")
            else:
                print(f"[ERROR] {task_name} → {result}")
                total_tokens += count_tokens_estimate(task)

    except Exception as e:
        print(f"[WARN] Native mode failed: {e}")
        print("[LOG]  Falling back to batch workflow execution (may fail with Ollama).")

        workflows = []
        for i, task in enumerate(CONCURRENT_TASK_PROMPTS):
            builder = graphbit.PyWorkflowBuilder(f"HiPerf Task {i+1}")
            node = graphbit.PyWorkflowNode.agent_node(
                name=f"HiPerf Task {i+1}",
                description=f"Executes task {i+1}",
                agent_id=agent_id,
                prompt=task,
            )
        ...
        for i, result in enumerate(results):
            ...
            task = CONCURRENT_TASK_PROMPTS[i]
            if result.is_completed() and not result.is_failed():
    ...
    print("[LOG]  High-performance concurrent run completed.")


if __name__ == "__main__":
    run_high_performance_concurrent_tasks_mistral()
