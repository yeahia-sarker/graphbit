    ...
    for model_id, model_name in models_to_test:
        try:
            ...
            task_node = Node.agent(
                name=f"{model_name} Assistant",
                prompt="Explain what makes artificial intelligence useful in 2-3 sentences.",
                agent_id=f"openrouter_{model_id.replace('/', '_').replace('-', '_')}"
            )
...
def demonstrate_multi_model_workflow():
    ...
    try:
        ...
        config = LlmConfig.openrouter(
            api_key=os.getenv("OPENROUTER_API_KEY"),
            model="anthropic/claude-3-5-haiku"  # Default model
        )
...
def demonstrate_openrouter_with_site_info():
    ...
    try:
        ...
        config = LlmConfig.openrouter_with_site(
            api_key=os.getenv("OPENROUTER_API_KEY"),
            model="openai/gpt-4o-mini",
            site_url="https://github.com/graphbit-ai/graphbit",
            site_name="GraphBit AI Framework"
        )
        ...
        task_node = Node.agent(
            name="Site-Aware Assistant",
            prompt="Explain the benefits of using a unified API for accessing multiple AI models.",
            agent_id="site_aware_assistant"
        )
